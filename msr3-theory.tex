\subsection{Relaxation of the mixed-effects variable selection model}
\label{sec:relax}
...
\subsection{Proximal Gradient Descent for $\uemu\tbg + R\tbg+\del_{\Rqp}(\tgam)$} 
\label{sec:pgd}
...
\begin{algorithm}[H]
\SetAlgoLined
$\tw = \tw^0$, $\gammax>\tgam^0$, $\alpha < \frac{2}{L}$ 
for $L>0$.\\
 \While{not converged and $\tgam\le\gammax$}{
    $\tw^+ = \prox_{\alpha \tR }(\tw - \alpha \nabla \uemu (\tw))$\\
 }
\caption{\label{alg:MSR3}Proximal Gradient Descent for 
$\Phi_\emu$}
\end{algorithm}
\medskip
...
to \eqref{eq:log-barrier problem} %\eqref{eq:vs_lme} 
