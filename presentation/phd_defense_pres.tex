...
1) We introduce physics to this system by adding a term which regularises latent gradient field. 
...
4) We feed a lot of collocation points and ask a network to do interpolation \end{frame}

\begin{frame}{Results: Extrapolation to Unknown Regimes}
1) We show that network can indeed interpolate between collocations. Moreover, it can fill the whole unknown regimes of behaviour. (Duffing example with explanation)
\end{frame}
