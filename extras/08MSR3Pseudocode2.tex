...
\begin{algorithm}[H]
\SetAlgoLined
$\texttt{progress}\leftarrow \textbf{True}$;
$\beta^+, \tbeta^+\leftarrow\beta_0$; \quad $\gamma^+, \tgamma^+\leftarrow\gamma_0$; \quad \texttt{outer\_iter = 0} \\
\While{\texttt{outer\_iter} $<$ \texttt{max\_iter}  \ and \ $\texttt{progress}$\\}
%($\|\tbeta^+ - \tbeta\| \geq \text{tol}$ or $\|\tgamma^+ - \tgamma\| \geq \text{tol}$) \\}
{
...
%	\tcp*[h]{Keep iterating until convergence} \\
 ...
 $\tbeta^+ = \prox_{\alpha R}(\beta^+)$; \quad $\tgamma^+ = \prox_{\alpha R + \delta_{\R_+}}(\gamma^+)$; \quad \texttt{outer\_iter += 1}; \quad
$ \texttt{progress}=$
($\|\tbeta^+ - \tbeta\| \geq \text{tol}$ or $\|\tgamma^+ - \tgamma\| \geq \text{tol}$) 
 }
 \Return{$\tbeta^+$, $\tgamma^+$}
 \caption{\label{alg:MSR3}MSR3 (Proximal Gradient Descent for the Value function)}
\end{algorithm}
